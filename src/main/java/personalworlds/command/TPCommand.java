    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if(args[0].equalsIgnoreCase("ls")) {
            CommonProxy.getDimensionConfigs(false).forEachEntry((dimID, dimCFG) -> {
                ...
                sender.sendMessage(new TextComponentTranslation(String.format("%d: %s", dimID, dimCFG.cloudsEnabled())));
                return true;
            });
            return;
        }
        if(args[0].equalsIgnoreCase("where")) {
            ...
            sender.sendMessage(new TextComponentTranslation("commands.pworlds.where", player.getName(), player.getEntityWorld().provider.getDimension()));
            return;
        }
        if(args[0].equalsIgnoreCase("tpx")) {
            ...
            sender.sendMessage(new TextComponentTranslation("commands.pworlds.tpx", player.getName(), dim, target.getX(), target.getY(), target.getZ()));
            return;
        }
